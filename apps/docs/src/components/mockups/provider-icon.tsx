...
const providerIcons: Record<string, React.ComponentType<{ className?: string }>> = {
  apple: createProviderImgIcon("/providers/apple.svg"),
  googlecalendar: createProviderImgIcon("/providers/googlecalendar.svg"),
...
export function ProviderIcon({ provider, className }: ProviderIconProps) {
  ...
  return (
    <span
      className={cn(
        "inline-block size-3.5 shrink-0 rounded bg-muted font-bold text-[8px] leading-[14px] text-center uppercase text-muted-foreground",
        className
      )}
      aria-hidden
    >
